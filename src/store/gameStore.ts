  ...
  subscribeWithSelector((set, get) => ({
    ...
    click: () => {
      ...
      set({
        resources: {
          ...
          totalLoCFromClicks:
            state.resources.totalLoCFromClicks + Math.max(0, value),
        },
        stats: {
          ...state.stats,
    ...
    tick: (deltaMs: number) => {
      ...
      set({
        resources: {
          ...
          techDebt: newTD,
          totalTechDebtEarned:
            (state.resources.totalTechDebtEarned ?? 0) + Math.max(0, tdChange),
        ...
        stats: {
          ...
          highestLocPerSecond: Math.max(
            state.stats.highestLocPerSecond,
            locPerSecond,
          ),
        },
        lastTickTimestamp: Date.now(),
    ...
    buyBuilding: (buildingId: string, quantity: number = 1) => {
      ...
      for (let i = 0; i < actualQty; i++) {
        totalCost += Math.floor(
          calculateBuildingCost(def, ownedCount + i) * costReduction,
        );
      }
      ...
      const newBuildings = state.buildings.map((b) =>
        b.id === buildingId ? { ...b, count: b.count + actualQty } : b,
      );
    ...
    shipProduct: () => {
      ...
      const newPrestige = {
        ...
        lifetimeLoCEarned:
          (state.prestige.lifetimeLoCEarned ?? 0) +
          state.resources.totalLoCEarned,
      };
    ...
    loadGame: () => {
      ...
      const elapsed = Math.min(
        (now - saved.lastTickTimestamp) / 1000,
        8 * 60 * 60,
      );
      ...
      const missingBuildings = BUILDINGS.filter(
        (b) => !existingIds.has(b.id),
      ).map((b) => ({ id: b.id, count: 0, totalProduced: 0 }));

      // Remove PM building if it exists in save
      const cleanedBuildings = [...saved.buildings, ...missingBuildings].filter(
      ...
      if (hack.effect.kind === "loc_burst") {
        ...
        const buff = {
          ...
          productionMultiplier:
            hack.effect.kind === "production_boost"
              ? hack.effect.multiplier
              : undefined,
          clickMultiplier:
            hack.effect.kind === "click_boost"
              ? hack.effect.multiplier
              : undefined,
